            ...
            if file.endswith(".md"):
                if(currentpath != path):
                    ...
                    if len(path.split(os.sep)) > 1:
                        ...
                        summary_md += f"* [{path.split(os.sep)[-1].lower()}]({path.replace(' ','-').replace(os.sep, '/')[2:]}/README.md)\n"
                    else:
                        summary_md += f"* [{path.split(os.sep)[-1][2:].lower()}]({path.replace(' ','-').replace(os.sep, '/')[2:]}/README.md)\n"
                
                if(path != directory):
                    # for i in path[len(directory):].split(os.sep):
                    summary_md += f"\t"
                summary_md += f"* [{file[0:-3].lower()}](<{os.path.join(path, file)[2:].replace(os.sep, '/')}>)\n"

    with open("SUMMARY.md", "w", encoding="utf-8") as f:
        f.write(summary_md[2:])
